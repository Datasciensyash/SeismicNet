...
def visualize_metrics(metrics, loss, plot=False, title='0'):
    ...
    if plot:
        ...
        plt.title(title)
        plt.plot(list(range(len(loss))), loss, c='r', alpha=0.8, label='loss')
        plt.plot(list(range(len(metrics))), metrics, c='g', alpha=0.8, label='IoU')
        plt.axhline(1, c='gray', label='Maximum possible val. of IoU', alpha=0.15)
...
if run := st.sidebar.button('Run experiments'):
